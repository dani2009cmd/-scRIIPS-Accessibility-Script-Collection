...
;   only while the selected game process is the foreground process.
; - Use "ATTACH ACTIVE" while the game window is in the foreground.
; ============================================================================

Global Const $APP_NAME = "UE5 Mouse Wheel Remapper"
Global Const $APP_VERSION = "v10.0 fixed"
...
While 1
    ...
    Switch GUIGetMsg()
        ...
        Case $idBtnAttach
            _AttachActiveProcess()

        Case $idBtnDetach
            $sTargetProcess = ""
            GUICtrlSetData($idInputProcess, "")
            _UpdateAttachedLabel()
            _UpdateStatusLabel()
    EndSwitch
WEnd
...
Func _SendRawUnrealInput($iVK, $iHoldMs)
    ...
    Local $tInput = DllStructCreate("DWORD type;BYTE union[" & $iUnionSize & "]")
...
Func _AttachActiveProcess()
    Local $hActive = $hLastExternalWindow
    If $hActive = 0 Then
        MsgBox($MB_ICONWARNING, $APP_NAME, "Előbb kattints a játék ablakára, majd térj vissza ide és nyomd meg az ATTACH ACTIVE gombot.")
        Return
    EndIf

    Local $iPID = WinGetProcess($hActive)
    If @error Or $iPID <= 0 Then
        MsgBox($MB_ICONWARNING, $APP_NAME, "Nem sikerült lekérni az aktív folyamatot.")
        Return
    EndIf

    Local $sName = _GetProcessNameByPID($iPID)
    If $sName = "" Then
        MsgBox($MB_ICONWARNING, $APP_NAME, "Nem sikerült lekérni az aktív folyamat nevét.")
        Return
    EndIf

    $sTargetProcess = StringLower($sName)
    GUICtrlSetData($idInputProcess, $sName)
    _UpdateAttachedLabel()
    _UpdateStatusLabel()
EndFunc
...
Func _TargetIsForeground()
    ; Keep the text box and internal target synchronized.
    If $idInputProcess Then $sTargetProcess = StringLower(StringStripWS(GUICtrlRead($idInputProcess), 3))

    ; Empty target = compatibility mode: remap in whichever window is active.
    If $sTargetProcess = "" Then Return True
    ...
    Local $sActiveName = _GetProcessNameByPID($iPID)
    If $sActiveName = "" Then Return False

    Return StringLower($sActiveName) = StringLower($sTargetProcess)
EndFunc

Func _UpdateAttachedLabel()
    If $sTargetProcess = "" Then
        GUICtrlSetData($idLblAttached, "TARGET: ANY FOREGROUND WINDOW")
        GUICtrlSetColor($idLblAttached, $COLOR_TEXT_MUTED)
    Else
        GUICtrlSetData($idLblAttached, "ATTACHED: " & $sTargetProcess)
...
Func _BuildGUI()
    ...
    $idInputProcess = GUICtrlCreateInput("", 160, 115, 210, 25)
    GUICtrlSetBkColor($idInputProcess, $COLOR_PANEL)
    GUICtrlSetColor($idInputProcess, $COLOR_NEON_CYAN)

    $idBtnAttach = GUICtrlCreateButton("ATTACH ACTIVE", 380, 114, 120, 27)
    GUICtrlSetFont($idBtnAttach, 8.5, 800, 0, "Segoe UI")

    $idBtnDetach = GUICtrlCreateButton("DETACH", 380, 145, 120, 25)
    GUICtrlSetFont($idBtnDetach, 8.5, 800, 0, "Segoe UI")

    $idLblAttached = GUICtrlCreateLabel("TARGET: ANY FOREGROUND WINDOW", 20, 150, 350, 20)
    ...
    Local $idInfo = GUICtrlCreateLabel("Tip: open the UE5 game, click it once, then press ATTACH ACTIVE.", 20, 560, 480, 40)
...
Func _UpdateStatusLabel()
    ...
    If $bEngineActive Then
        If $sTargetProcess = "" Then
            GUICtrlSetData($idStatusLabel, "READY - remapper active on foreground window")
