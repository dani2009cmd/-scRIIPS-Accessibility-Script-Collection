; ============================================================
; Combo ReMapper 8.8.7 — TRAY MENU, EXPORT/IMPORT & OSD LOCK
...
RegisterPanicHotkey() {
    ...
    if (panicKey != "") {
        try {
            ...
            MsgBox("Failed to bind panic key: " . panicKey, "Combo ReMapper 8.8.7", "Icon!")
        }
    }
}
...
RegisterOsdHotkey() {
    ...
    if (osdToggleKey != "") {
        try {
            ...
            MsgBox("Failed to bind OSD toggle key: " . osdToggleKey, "Combo ReMapper 8.8.7", "Icon!")
        }
    }
}
...
ExportProfile(*) {
    ...
    outStr := "[REMAPPER_v8.8.7|NAME:" . currentProfile . "|EXE:" . targetExe . "|PANIC:" . panicKey . "]`n"
    for row in rows {
        if (row.trigger != "")
...
; ============================================================
BuildGUI() {
    global myGui, rowUI, rowSlider, scrollLabel, panicKeyBox, scrollKeyBox, targetExeBox, applyBtn, toggleBtn, statusBadge, statusText
    ...
    myGui := Gui("+Resize", "Combo ReMapper 8.8.7")
    myGui.SetFont("s10")
...
OpenSettingsPanel(*) {
    ...
    SaveSettingsAction() {
        global keyDelayDuration, keyDelayPress, mouseDelayDuration, soundAlerts, showTooltips, showOsd, osdLocked, osdToggleKey
        global customBgColor, customControlColor, customTextColor, Themes, osdGui, scriptEnabled
...
ApplyChanges(*) {
    ...
    if (panicKey = "") {
        MsgBox("The panic toggle key cannot be blank. This is required for safety.", "Combo ReMapper 8.8.7", "Icon!")
        return
    }
    ...
    for row in rows {
        ...
        if (cleanTrig = StrUpper(panicKey)) {
            MsgBox("'" row.trigger "' is reserved for the panic toggle and can't be a trigger key. Nothing was saved.", "Combo ReMapper 8.8.7", "Icon!")
            return
        }
        if (seenTriggers.Has(cleanTrig)) {
            MsgBox("'" row.trigger "' is used as a trigger key on more than one row. Please make each trigger key unique.", "Combo ReMapper 8.8.7", "Icon!")
            return
        }
    ...
    if (scrollTriggerKey != "") {
        if (StrUpper(scrollTriggerKey) = StrUpper(panicKey)) {
            MsgBox("The scroll-click key can't be the panic toggle. Nothing was saved.", "Combo ReMapper 8.8.7", "Icon!")
            return
        }
        if (seenTriggers.Has(StrUpper(scrollTriggerKey))) {
            MsgBox("The scroll-click key ('" scrollTriggerKey "') is the same as one of your row trigger keys. Pick a different one.", "Combo ReMapper 8.8.7", "Icon!")
            return
        }
    }
