...
; Sets native "cue banner" placeholder text on an Edit control - shows only
; while the field is empty, vanishes the moment the user types, so every
; still-blank sequence/hotkey box reads as in-universe UAA terminal flavor
; instead of a plain empty box. wParam=1 keeps it visible even while focused.
SetCueBanner(editCtrl, text) {
    SendMessage(0x1501, 1, StrPtr(text), editCtrl.Hwnd)   ; EM_SETCUEBANNER
}
...
BuildMainGui() {
    ...
    MainGui.Add("Text", "x10 y12 c" ClrRed, "Game profile:")
    ...
    SetCueBanner(exeBox, "e.g. rpcs3.exe")
    ...
    StatusText := MainGui.Add("Text", "x10 y40 w870 cGreen", "")
    ...
    SetCueBanner(dbBox, "NO DEATH BLOW SEQUENCE ON FILE, ASSASSIN - LOG ONE VIA DETECT")
    ...
    SetCueBanner(krBox, "RECHARGE SEQUENCE UNLOGGED FOR THIS ENGINE")
...
PromptExploreEdit(row) {
    ...
    SetCueBanner(hkBox, "NO HOTKEY BOUND")
    ...
    SetCueBanner(keysBox, "NO SEQUENCE ON FILE - DETECT OR TYPE ONE")
...
PromptComboEdit(row) {
    ...
    SetCueBanner(trigBox, "NO TRIGGER BOUND")
    ...
    SetCueBanner(keysBox, "NO COMBO ON FILE - DETECT OR TYPE ONE")
