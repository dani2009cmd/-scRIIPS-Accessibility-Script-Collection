...
; Remove the maximize box so the window can't be maximized (block maximize)
hwnd := MyGui.Hwnd
WinSetStyle("-0x10000", "ahk_id " hwnd) ; WS_MAXIMIZEBOX

MyTab := MyGui.Add("Tab3", "x10 y10 w900 h140 vMainTab", ["Brush", "View / Canvas", "Layers", "Edit / Select", "Custom"])
...
; ---------------------- Bottom control row ----------------------
SnapBtn := MyGui.Add("Button", "x10 y160 w140 h28", "Snap to Bottom")
SnapBtn.OnEvent("Click", SnapToBottom)

TopChk := MyGui.Add("Checkbox", "x160 y163 w110 h22 cWhite Checked", "Always on Top")
TopChk.OnEvent("Click", ToggleAlwaysOnTop)

MyGui.Add("Text", "x290 y164 w60 cWhite", "Opacity:")
OpacitySlider := MyGui.Add("Slider", "x350 y160 w150 h28 Range80-255", CurrentOpacity)
OpacitySlider.OnEvent("Change", ChangeOpacity)

HideNote := MyGui.Add("Text", "x520 y164 cWhite", "F9 = show/hide    F10 = release Pan/held keys")

ExitBtn := MyGui.Add("Button", "x760 y160 w150 h28", "Exit")
ExitBtn.OnEvent("Click", (*) => ExitApp())

; ---------------------- Show + position ----------------------
MyGui.Show("w920 h" BarHeight)
...
SnapToBottom(*) {
    ...
    w := R - L
    MyGui.Move(L, B - BarHeight, w, BarHeight)
}
...
GuiResize(guiObj, MinMax, Width, Height) {
    ...
    try {
        ...
        tabCtrl.Move(, , Width - 20, Height - 60)
    }
}
