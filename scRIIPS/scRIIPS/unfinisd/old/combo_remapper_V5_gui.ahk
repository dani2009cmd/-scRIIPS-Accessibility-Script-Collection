; ============================================================
; Combo Remapper — GUI VERSION (WITH TRIGGER-IGNORING AUTO RECORDER)
...
rowUI := []                ; Array of 6 slot objects: {tb, cb, autoBtn, dd, addBtn, remBtn}
rowSlider := ""
scrollLabel := ""
panicKeyBox := ""
scrollKeyBox := ""
...
StartKeyRecorder(slotIdx, *) {
    ...
    ; Setup non-blocking InputHook for 5 seconds
    ih := InputHook("V L0 Timeout5")
    ih.KeyOpt("{All}", "+N")
    
    ih.OnKeyDown := (hook, vk, sc) => (
        keyName := StrLower(GetKeyName(Format("vk{:x}sc{:x}", vk, sc))),
        ; Ignore if key is empty, already recorded, OR matches the row's trigger key
...
BuildGUI() {
    ...
    ; Headers
    myGui.Add("Text", "xm ym w80", "Trigger key")
    myGui.Add("Text", "x+5 yp w150", "Holds these keys")
    myGui.Add("Text", "x+52 yp w65", "Mode")
    ...
    loop VISIBLE_ROWS {
        ...
        addBtn := myGui.Add("Button", "x+2 yp w24 h22", "+")
        remBtn := myGui.Add("Button", "x+2 yp w55 h22", "Rem")
        ...
        rowUI.Push({tb: tb, cb: cb, autoBtn: autoBtn, dd: dd, addBtn: addBtn, remBtn: remBtn})
    }

    ; Vertical side scrollbar slider
    rowSlider := myGui.Add("Slider", "x485 y35 h175 Vertical Range0-0", 0)
