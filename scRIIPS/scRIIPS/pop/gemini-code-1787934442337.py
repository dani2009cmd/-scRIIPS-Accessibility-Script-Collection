...
def main():
    try:
        ...
        # List of utility JSON files to ignore
        IGNORE_FILES = [
            "last_patch_report.json",
            "config.json",
            "settings.json",
            "report.json",
        ]
        ...
        if not json_files:
            log(
                "ERROR: No actual dialogue .json files found! Make sure your text files are in this folder."
            )
            return

        log(
            f"Found {len(json_files)} script file(s) to process: {', '.join(json_files)}"
        )
        ...
        for target_file in json_files:
            ...
            out_name = os.path.join(
                SCRIPT_DIR, f"{os.path.splitext(target_file)[0]}_FIXED.json"
            )
            with open(out_name, "w", encoding="utf-8") as f:
                json.dump(repaired, f, ensure_ascii=False, indent=2)

            log(f"SUCCESS! Processed {count} sentences in {target_file}.")
            log(f"Saved fixed file to: {out_name}")
